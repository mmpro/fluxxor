...
Dispatcher.prototype.processQueue = function() {
  ...
  if (queueSize === 0) {
    interval = 100
    // for IE <= 11 -> make sure timer is not called too often
    if (!!window.MSInputMethodContext && !!document.documentMode) interval =  400
  }
  else {
    // for IE <= 11
    if (!!window.MSInputMethodContext && !!document.documentMode) interval =  100
  }
  ...
  setTimeout(this.processQueue.bind(this), interval)
}
...
Dispatcher.prototype.dispatch = function(action) {
  // put every action into a queue - the queue will be processed sequentially by the setInterval function above
